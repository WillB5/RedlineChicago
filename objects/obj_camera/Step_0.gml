...
var middlePointX = room_width/2;
   ...
   // get 0-100%
    perc  = point_distance(obj_playerMike.x, obj_playerMike.y, middlePointX, obj_playerMike.y ) / middlePointX;
   var flip = 1;
   if (obj_playerMike.x > middlePointX ) {  flip = -1; }
   if (obj_playerMike.x < middlePointX ) {  flip =  1; }  
   var bufferX = 100;
    if (obj_playerMike.x < (middlePointX + bufferX)  and obj_playerMike.x > (middlePointX - bufferX)  ) {  
	flip = 0;	
	}
